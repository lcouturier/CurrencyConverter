...
namespace CurrencyConverter
{
	...
	public static class StringExtensions
	{
		...
		/// <summary>
		/// Tries from application settings.
		/// </summary>
		/// <typeparam name="T"></typeparam>
		...
		[SuppressMessage("Microsoft.Design", "CA1062:Valider les arguments de m�thodes publiques", MessageId = "1"),
		 SuppressMessage("Microsoft.Design", "CA1031:DoNotCatchGeneralExceptionTypes")]
		public static Option<T> TryFromAppSettings<T>(this string key)
			where T : IConvertible
		{
			Contract.Requires<ArgumentNullException>(!string.IsNullOrEmpty(key));
			Contract.Ensures(Contract.Result<Option<T>>() != null);

			var getKey = Fun.Create((string x) => ConfigurationManager.AppSettings[x]).ReturnOption().OnExceptionNone()
				.Memoize();

			Func<Func<object>, T> convert = x => ConvertTo<T>(x());
			var f = convert.Curry(() => getKey(key).Value).ReturnOption().OnExceptionNone();

			return getKey(key).HasValue ? f() : Option.None;
		}
